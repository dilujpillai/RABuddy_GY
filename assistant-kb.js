...
(function () {
    ...
    const KB = {
        ...
        buttons: [
            ...
            { label: '\ud83c\udfaf Suggest Closest Match', where: 'above the table', does: 'For any Hazard Group / Hazard List dropdown still outlined red after AI Fix, applies the closest standard entry as a best guess for you to review. Purely local - it never calls the AI. If the guesses are bad, the "\u21a9\ufe0f Undo Suggestions" button appears right next to it to revert them.', dom: 'aiClosestMatchBtn' },
            { label: '\u21a9\ufe0f Undo Suggestions', where: 'above the table, appears next to Suggest Closest Match right after it runs', does: 'Reverts every Hazard Group / Hazard List value that the last Suggest Closest Match run changed, back to what it was before. Hidden until Suggest Closest Match actually changes something, and only covers that one run - running Suggest Closest Match again replaces what Undo can revert.', dom: 'undoClosestMatchBtn' },
